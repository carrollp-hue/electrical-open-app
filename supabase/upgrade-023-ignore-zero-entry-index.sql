-- Legacy workbook rows used 0 where the handicap index at entry was unknown.
-- A zero must not become the player's baseline index, otherwise the +5 hard cap
-- incorrectly limits the calculated index to 5.0.
...
-- Prefer a calculated snapshot. The workbook seed remains the fallback for a
-- player who has no qualifying score history yet.
...
-- Refresh the latest snapshot for every player with qualifying history.
