-- Automatic society-index recalculation. Run after upgrade-001.sql.
alter table public.players add column if not exists club_handicap numeric(4,1);
alter table public.players add column if not exists committee_adjustment numeric(4,1) not null default 0;
...
declare
  v_count integer;
  v_used integer;
  v_rule_adjustment numeric;
  v_average numeric;
  v_pre_cap numeric;
  v_baseline numeric;
  v_soft_cap numeric;
  v_hard_cap numeric;
  v_soft_reduction numeric := 0;
  v_calculated numeric;
  v_club_handicap numeric;
  v_committee_adjustment numeric;
  v_score_adjustments numeric;
  v_result numeric;
  v_selected numeric[];
begin
  select club_handicap, committee_adjustment into v_club_handicap, v_committee_adjustment from public.players where id = p_player_id;
  with latest as (
  ...
  ) select count(*), min(handicap_index_at_entry), coalesce(sum(esr_adjustment + winner_cut), 0) into v_count, v_baseline, v_score_adjustments from latest;
...
begin
  if new.score_differential is not null then perform public.recalculate_society_index(new.player_id, new.fixture_id); end if;
  return new;
end $$;

drop trigger if exists calculate_society_index_after_score on public.fixture_entries;
create trigger calculate_society_index_after_score after insert or update of score_differential, esr_adjustment, winner_cut, handicap_index_at_entry on public.fixture_entries for each row execute function public.trigger_recalculate_society_index();
