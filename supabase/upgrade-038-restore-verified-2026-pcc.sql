-- The original workbooks confirm their recorded PCC values were already correct.
-- Differences from workbook differentials are caused by the app's deliberate
-- par-plus-two per-hole cap, not by PCC. This supersedes upgrade 037.

begin;

update public.fixtures
set playing_conditions_adjustment = case
  when fixture_date = '2026-03-29'::date and name = 'Stockwood Park' then 2
  when fixture_date = '2026-04-19'::date and name = 'Little Hay' then 2
  when fixture_date = '2026-08-02'::date and name = 'Oakland Park' then 1
end
