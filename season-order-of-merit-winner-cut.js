(() => {
  ...
  async function addControl() {
    ...
    const render = async year => {
      ...
      const { data: applied, error } = await client.from('season_order_of_merit_winner_cuts').select('season_year, player_id, amount, players(first_name, surname)').eq('season_year', year).maybeSingle();
