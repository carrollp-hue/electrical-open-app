...
handicap = function (roundId) {
  ...
  const form = text => `<section class="section club-handicap-reminder"><div class="section-head"><h2>Add/Modify Club Handicap</h2><button class="secondary" type="button" id="club-handicap-toggle" aria-expanded="false">Open</button></div><form class="admin-form club-handicap-editor" id="club-handicap-self-form" hidden><p class="intro">${text}</p><label>Club handicap<input name="club_handicap" type="number" min="0" max="54" step="0.1" inputmode="decimal" value="${club == null ? '' : Number(club).toFixed(1)}" required></label><p class="form-message" id="club-handicap-self-message"></p><button class="primary" type="submit">Save club handicap</button></form></section>`;
  const reminder = reminderDue && club != null
    ? form(`You are playing ${esc(upcomingFixture.name)} on ${date(upcomingFixture.fixture_date)}. Your club handicap was last confirmed on ${formattedSubmitted}; update it if it has changed, or confirm the same value to keep it current.`)
    : isOwnProfile
      ? form(club == null ? 'Enter your current club handicap if you hold one. Your lower club or society handicap will be used for fixture calculations.' : `Your club handicap was last submitted on ${formattedSubmitted}. You can update it whenever it changes.`)
      : '';
