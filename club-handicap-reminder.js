...
handicap = function (roundId) {
  ...
  const form = text => `<section class="section club-handicap-reminder"><div class="section-head"><h2>Add/Modify Club Handicap</h2><button class="secondary" type="button" id="club-handicap-toggle" aria-expanded="false">Open</button></div><p class="intro">${text}</p><form class="admin-form club-handicap-editor" id="club-handicap-self-form" hidden><label>Club handicap<input name="club_handicap" type="number" min="0" max="54" step="0.1" inputmode="decimal" value="${club == null ? '' : Number(club).toFixed(1)}" required></label><p class="form-message" id="club-handicap-self-message"></p><button class="primary" type="submit">Save club handicap</button></form></section>`;
