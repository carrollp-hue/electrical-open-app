(() => {
  ...
  fixtures = function (fixtureId) {
    ...
    return `<p class="eyebrow">Society calendar</p><h1>Fixtures & results</h1><section class="section fixture-list-section"><div class="fixture-list-heading"><h2>Fixtures</h2><span>Upcoming</span></div>${upcoming.length ? groupedByYear(upcoming) : empty('No upcoming fixtures.')}</section><div class="fixture-results-divider" aria-hidden="true"></div><section class="section fixture-list-section results-list-section"><div class="fixture-list-heading"><h2>Results</h2><span>Completed</span></div>${results.length ? groupedByYear(results) : empty('No completed fixtures yet.')}</section>${historicalCount ? `<section class="section historical-archive-link"><h2>Historical results</h2><p>Browse ${historicalCount} verified archived fixture${historicalCount === 1 ? '' : 's'} with gross, nett and Stableford points.</p>${link('fixtures/historical', 'Open historical archive')}</section>` : ''}`;
  };
