(() => {
  ...
  home = function () {
    ...
    const updateList = updates.length
      ? `<table class="table"><tbody>${updates.map(update => `<tr><td><strong>${esc(update.title)}</strong><br><span>${esc(update.body)}</span><br><small>${dateTime(update.created_at)}</small></td><td>${update.url ? `<a class="text-link" href="#${String(update.url).split('#')[1] || 'home'}">View</a>` : ''}</td></tr>`).join('')}</tbody></table>`
